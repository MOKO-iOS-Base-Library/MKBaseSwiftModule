...
class MKTextSwitchCellModel {
    ...
    func cellHeight() -> CGFloat {
        var msgWidth = screenWidth - 3 * 15 - 40
        if leftIcon != nil {
            msgWidth = msgWidth - leftIcon!.size.width - 3
        }
...
protocol MKTextSwitchCellDelegate: AnyObject {
    
    /// 右侧按钮点击事件
    /// - Parameters:
    ///   - index: 当前cell所在的index
    ///   - isOn: 当前开关状态
    func mk_textSwitchCellStatusChanged(index: Int, isOn: Bool)
}

class MKTextSwitchCell: MKBaseCell {
    var delegate: MKTextSwitchCellDelegate?
    var dataModel: MKTextSwitchCellModel? {
        get {
    ...
    static func initCell(tableView: UITableView) -> MKTextSwitchCell {
        let reuseIdentifier = "MKTextSwitchCellIdenty"
        if let cell = tableView.dequeueReusableCell(withIdentifier: reuseIdentifier) as? MKTextSwitchCell {
            return cell
        } else {
            return MKTextSwitchCell(style: .default, reuseIdentifier: reuseIdentifier)
        }
    }
    ...
    override func layoutSubviews() {
        ...
        msgLabel.snp.remakeConstraints { make in
            ...
            make.right.equalTo(selectedButton.snp.left).offset(-15)
            if hasNote {
                make.top.equalTo(15)
            }else {
                make.centerY.equalTo(contentView.snp.centerY)
    ...
    @objc func selectedButtonPressed() {
        ...
        let icon = (selectedButton.isSelected ? LOADIMAGE("mk_switchSelectedIcon", "png") : LOADIMAGE("mk_switchUnselectedIcon", "png"))
        selectedButton.setImage(icon, for: .normal)
        self.delegate?.mk_textSwitchCellStatusChanged(index: dataModel!.index,isOn: selectedButton.isSelected)
    }
    ...
    private func updateDataModel() {
        ...
        selectedButton.isEnabled = dataModel!.enable
        selectedButton.isSelected = dataModel!.isOn
        let icon = (selectedButton.isSelected ? LOADIMAGE("mk_switchSelectedIcon", "png") : LOADIMAGE("mk_switchUnselectedIcon", "png"))
        selectedButton.setImage(icon, for: .normal)
    ...
    private var privateDataModel: MKTextSwitchCellModel?
...
extension MKTextSwitchCell {
    private func msgSize() -> CGSize{
        if !ValidStr(self.dataModel?.msg) {
            return CGSize(width: 0, height: 0)
        }
        var maxMsgWidth = (contentView.frame.size.width - 3 * 15 - 40)
        if self.dataModel!.leftIcon != nil {
            maxMsgWidth = maxMsgWidth - self.dataModel!.leftIcon!.size.width - 3
        }
